...
games <- 30
rounds <- 15
fileNamePrefix <- "./RQ21-ImpactOfTheNumberOfCardsPerDeckWith10Rounds/data/NSGAII/"
...
plot <- ggplot(RQ21,aes(x=DeckSize,y=Fairness))+
  geom_boxplot() +
  labs(x="Deck size (number of cards)")
ggsave(file="ImpactOfDeckSizeOnFairnessBis.pdf",plot,device="pdf")

plot<- ggplot(RQ21,aes(x=DeckSize,y=Closeness))+
  geom_boxplot() +
  labs(x="Deck size (number of cards)")
ggsave(file="ImpactOfDeckSizeOnClosenessBis.pdf",plot,device="pdf")

plot <- ggplot(RQ21,aes(x=RQ21$DeckSize,y=DominatedCards))+
  geom_boxplot() +
  labs(x="Deck size (number of cards)",y="Number of dominated cards")
ggsave(file="ImpactOfDeckSizeOnDominatedCardsBis.pdf",plot,device="pdf")
